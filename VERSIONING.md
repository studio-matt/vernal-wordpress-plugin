...
- The plugin checks for updates about every **1 hour** (and also refreshes when an admin opens the Plugins screen, at most hourly)
- Users can manually check by clicking "Check for updates" on the Plugins page (forces an immediate GitHub poll)
- There is no push/broadcast from GitHub to WordPress — sites poll on that interval
